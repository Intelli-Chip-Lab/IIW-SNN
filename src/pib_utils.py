...
def train_SGLD(args, model, optimizer, criterion, scheduler, train_data, device, epoch, energy_decay, ):
    model.train()
    ...
    for step, (inputs, targets) in enumerate(train_data):
        ...
        if epoch > 0:
            (energy_decay + avg_loss - avg_loss).backward(retain_graph=True)  # retain_graph=True useage:  https://blog.csdn.net/qq_24502469/article/details/104559250
            avg_loss.backward()
        else:
            avg_loss.backward()
        optimizer.step()
...
def train_pib(args,model,
              sub_idx, x_tr, y_tr, x_va, y_va, x_te, y_te, num_epoch, batch_size, lr, weight_decay,
              beta, early_stop_ckpt_path, early_stop_tolerance=10,
              schedule=[50, 80, 100], gamma=0.1, noise_scale=1e-4,
              pretrain_step=20,
              ):
    info_dict, loss_acc_dict = train_SGLD(model,sub_idx, x_tr, y_tr, x_va, y_va, num_epoch=num_epoch, batch_size=batch_size, lr=lr, weight_decay=weight_decay, beta=beta,
                                          early_stop_ckpt_path=early_stop_ckpt_path, early_stop_tolerance=early_stop_tolerance, noise_scale=noise_scale,  schedule=schedule, gamma=gamma,)
...
def train_iiw(model,
    ...
    for epoch in range(num_epoch):
        ...
        if verbose:
            ...
            print("epoch: {}, tr loss: {}, lr: {}, e_decay: {}".format(epoch, total_loss / num_all_tr_batch, lr,
                                                                       energy_decay))
